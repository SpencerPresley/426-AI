...
from langchain.evaluation import load_evaluator, EmbeddingDistance, EmbeddingDistanceEvalChain
...
class EmbeddingAnalysis:
    def __init__(
        ...
        self.model = model
        self.openai_embeddings = OpenAIEmbeddings(openai_api_key=self.api_key, model=self.model)
        self.embeddings = {}
    ...
    def _calculate_all_similarities(self):
        ...
        for (key1, emb1_list), (key2, emb2_list) in combinations(self.embeddings.items(), 2):
            print(f"Comparing {key1} and {key2}")  # Debugging: Print each comparison being generated

            similarities = []
            for emb1 in emb1_list:
                for emb2 in emb2_list:
                    print(f"Running evaluators for list items: {emb1[:5]}... and {emb2[:5]}...")  # Debugging

                    # Calculate multiple metrics
                    cosine_result = self._run_evaluator(self.cosine_similarity_evaluator, emb1, emb2)
                    euclidean_result = self._run_evaluator(self.euclidean_distance_evaluator, emb1, emb2)
                    manhattan_result = self._run_evaluator(self.manhattan_distance_evaluator, emb1, emb2)
                    chebyshev_result = self._run_evaluator(self.chebyshev_distance_evaluator, emb1, emb2)
                    hamming_result = self._run_evaluator(self.hamming_distance_evaluator, emb1, emb2)

                    # Normalize distance metrics (optional, depending on your scale)
                    normalized_euclidean = 1 / (1 + euclidean_result['score']) if euclidean_result and 'score' in euclidean_result else None
                    ...
                    if cosine_result and 'score' in cosine_result:
                        combined_score = (cosine_weight * cosine_result['score']) + \
                                        (distance_weight * normalized_euclidean if normalized_euclidean is not None else 0)
                    ...
                    if cosine_result and combined_score is not None:
                        percentage_drop = ((cosine_result['score'] - combined_score) / cosine_result['score']) * 100

                    # Store the pair of items and their similarity scores
                    similarities.append({
                        "item1": emb1,
                        "item2": emb2,
                        "cosine": cosine_result['score'] if cosine_result and 'score' in cosine_result else None,
                        "euclidean": euclidean_result['score'] if euclidean_result and 'score' in euclidean_result else None,
                        "manhattan": manhattan_result['score'] if manhattan_result and 'score' in manhattan_result else None,
                        "chebyshev": chebyshev_result['score'] if chebyshev_result and 'score' in chebyshev_result else None,
                        "hamming": hamming_result['score'] if hamming_result and 'score' in hamming_result else None,
        ...
        for comparison_key in comparisons:
            if comparison_key in self.similarity_results:
                ...
                if "individual_similarities" in results:
                    table = []
                    table_data = []
                    headers = ["String 1", "String 2", "Cosine", "Euclidean", "Manhattan", "Chebyshev", "Hamming", "Combined", "Percentage Drop"]
                    ...
                    for similarity in results["individual_similarities"]:
                        ...
                        table.append([
                            ...
                            f"{combined_score:.2f}%" if combined_score is not None else "N/A",
                            f"{percentage_drop:.2f}%" if percentage_drop is not None else "N/A"  # Add percentage drop to the table
                        ])
                        ...
                        table_data.append([
                            ...
                            f"{hamming:.2f}" if hamming is not None else "N/A",
                            f"{combined_score:.2f}%" if combined_score is not None else "N/A",
                            f"{percentage_drop:.2f}%" if percentage_drop is not None else "N/A"  #
                        ])
                    ...
                    fig, ax = plt.subplots(figsize=(12, len(table) * 0.5 + 1)) # Adjust height based on number of rows
                    ax.axis('tight')
                    ax.axis('off')  
                        
                    # Add the title (String 1 and String 2) above the table
                    fig.text(0.5, 0.85, f"String 1: {item1[:50]}...", ha='center', fontsize=12, weight='bold')
                    fig.text(0.5, 0.75, f"String 2: {item2[:50]}...", ha='center', fontsize=12, weight='bold')
                        
                    table_plt = ax.table(cellText=np_table, colLabels=headers[2:], cellLoc='center', loc='center')
                    ...
                    with open(self.file_name, mode) as file:
                        ...
                        file.write(tabulate(table, headers=["String 1", "String 2", "Cosine", "Euclidean", "Manhattan", "Chebyshev", "Hamming", "Combined", "Percentage Drop"], tablefmt="grid"))
                        file.write("\n\n\n") 
                        print(f"Table written to file: {self.file_name}")
                        
                    # Print the table with the new column
                    print(f"Comparison: {comparison_key}")
                    print(tabulate(table, headers=["String 1", "String 2", "Cosine", "Euclidean", "Manhattan", "Chebyshev", "Hamming", "Combined", "Percentage Drop"], tablefmt="grid"))
...
if __name__ == "__main__":
    ...
    analysis = EmbeddingAnalysis(api_key=openai_api_key, file_name="accurate_similarity_results.txt", plt_prefix="analysis_1_", save_dir='Plots/Analysis_1')
    ...
    analysis_2 = EmbeddingAnalysis(api_key=openai_api_key, file_name="inaccurate_similarity_results.txt", plt_prefix="analysis_2_", save_dir='Plots/Analysis_2')
    ...
    analysis_3 = EmbeddingAnalysis(api_key=openai_api_key, file_name="inaccurate_similarity_results_2.txt", plt_prefix="analysis_3_", save_dir='Plots/Analysis_3')
    ...
    analysis_3.embed_strings(
        ...
        title=["Changes in foreign ownership and innovation investment: the case of Korean corporate governance reforms"],
    ...
    analysis_4 = EmbeddingAnalysis(api_key=openai_api_key, file_name="inaccurate_similarity_results_3.txt", plt_prefix="analysis_4_", save_dir='Plots/Analysis_4')
